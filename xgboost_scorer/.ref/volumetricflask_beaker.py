...
def get_beaker_to_flask_confidence(img: np.ndarray) -> float:
    """
    一个完全自包含的函数，用于预测“烧杯->容量瓶”倾倒动作的置信度。
    ...
    float: 介于0.0到100.0之间的置信度分数。
           如果发生错误，返回0.0。
    """
    global _MODELS_CACHE_BF

    # --- 第一部分：初始化与设置 (只在第一次调用时执行) ---
    if not _MODELS_CACHE_BF:
        print("首次调用 (烧杯->容量瓶)，正在加载所有模型，请稍候...")
        try:            
            _MODELS_CACHE_BF['YOLO_MODELS'] = {
                'box_model': YOLO(r'./model/yolov11-vessels.pt'),
                'pose_models': {
                    'beaker': YOLO(r'./model/yolov11-pose-beaker.pt'),
                    'volumetric_flask': YOLO(r'./model/yolov11-pose-volumetricflask.pt')
                }
            }
            _MODELS_CACHE_BF['HANDS_DETECTOR'] = mp.solutions.hands.Hands(
                static_image_mode=True, max_num_hands=2, min_detection_confidence=0.5)
            
            xgb_model = xgb.XGBClassifier()
            # 推荐使用 .ubj 二进制格式以避免编码错误
            xgb_model_path = r"./model/xgboost_pour_model_volumetricflask_beaker.json"

            xgb_model.load_model(xgb_model_path)
            _MODELS_CACHE_BF['XGB_MODEL'] = xgb_model

            print("所有模型加载成功并已缓存！")
        except Exception as e:
            print(f"严重错误：模型加载失败！请检查所有模型文件路径是否正确。")
            print(f"错误详情: {e}")
            _MODELS_CACHE_BF.clear() # 加载失败，清空缓存
            return 0.0

    # --- 定义常量 ---
    BOX_CONFIDENCE_THRESHOLD = 0.5
    KEYPOINT_VISIBILITY_THRESHOLD = 0.05
    VESSEL_IDS = {'beaker': 1, 'flask': 2}
    KEYPOINT_IDS = {
    ...
    def initial_detection(image, box_model, hands_detector):
        all_detected_beakers, all_detected_flasks = [], []
        ...
        for r in box_results[0].boxes:
            if r.conf[0] > BOX_CONFIDENCE_THRESHOLD:
                ...
                if int(r.cls[0]) == VESSEL_IDS['beaker']: all_detected_beakers.append(item)
        ...
        return all_detected_beakers, all_detected_flasks, detected_hands
    def select_target_pair(beakers, flasks, hands):
        if not beakers and flasks: return None, flasks[0]
        if beakers and not flasks: return beakers[0], None
        if not beakers and not flasks: return None, None
        if len(beakers) == 1 and len(flasks) == 1: return beakers[0], flasks[0]
        if len(hands) == 2:
            min_dist, best_pair = float('inf'), (None, None)
            for bk in beakers:
                for flsk in flasks:
                    b_c, f_c = get_box_center(bk['box']), get_box_center(flsk['box'])
                    h0_c, h1_c = hands[0]['center'], hands[1]['center']
                    dist1 = np.linalg.norm(f_c - h0_c) + np.linalg.norm(b_c - h1_c)
                    dist2 = np.linalg.norm(f_c - h1_c) + np.linalg.norm(b_c - h0_c)
                    if min(dist1, dist2) < min_dist: min_dist, best_pair = min(dist1, dist2), (bk, flsk)
            return best_pair[0], best_pair[1]
        elif len(hands) == 1:
            h_c, selected_flask, selected_beaker = hands[0]['center'], None, None
            all_vessels = beakers + flasks
            ...
            if closest_vessel in flasks:
                ...
                if beakers: selected_beaker = min(beakers, key=lambda b: np.linalg.norm(get_box_center(b['box']) - get_box_center(selected_flask['box'])))
            else:
                selected_beaker = closest_vessel
                if flasks: selected_flask = min(flasks, key=lambda f: np.linalg.norm(get_box_center(f['box']) - get_box_center(selected_beaker['box'])))
            return selected_beaker, selected_flask
        else: # 0 hands
            min_dist, best_pair = float('inf'), (None, None)
            for bk in beakers:
                for flsk in flasks:
                    dist = np.linalg.norm(get_box_center(bk['box']) - get_box_center(flsk['box']))
                    if dist < min_dist: min_dist, best_pair = dist, (bk, flsk)
            return best_pair[0], best_pair[1]
    ...
    def extract_features(beaker, flask, hands):
        features = {f'f{i}': -1 for i in range(1, 19)}
        for i in range(11, 18): features[f'f{i}'] = 0
        features['f1'] = 1 if beaker else 0
        features['f2'] = 1 if flask else 0
        if features['f1'] and features['f2']:
            beaker_center, flsk_center = get_box_center(beaker['box']), get_box_center(flask['box'])
            beaker_diag = get_diagonal(beaker['box'])
            features['f3'] = np.linalg.norm(beaker_center - flsk_center) / beaker_diag if beaker_diag > 0 else -1
            bk_kps, flsk_kps = beaker.get('keypoints'), flask.get('keypoints')
            if bk_kps is not None and flsk_kps is not None:
                if bk_kps['conf'][KEYPOINT_IDS['beaker']['spout']] > KEYPOINT_VISIBILITY_THRESHOLD and flsk_kps['conf'][KEYPOINT_IDS['flask']['mouth']] > KEYPOINT_VISIBILITY_THRESHOLD:
                    bk_spout_pt, flsk_mouth_pt = bk_kps['xy'][KEYPOINT_IDS['beaker']['spout']], flsk_kps['xy'][KEYPOINT_IDS['flask']['mouth']]
                    features['f4'] = np.linalg.norm(bk_spout_pt - flsk_mouth_pt) / beaker_diag if beaker_diag > 0 else -1
                    features['f5'] = 1
        ...
        if features['f2'] and features['f6'] > 0:
            ...
            if features['f6'] == 2:
                ...
                if features['f1']: dist_bk_hand = np.linalg.norm(hands[0]['center'] - get_box_center(beaker['box']))
                if dist_flsk_hand < dist_bk_hand:
                     hand_diag = get_diagonal(closest_hand_flsk['box'])
                     features['f8'] = dist_flsk_hand / hand_diag if hand_diag > 0 else -1
        if features['f2']:
        ...
        if features['f1'] and beaker.get('keypoints') is not None:
            kps_conf = beaker['keypoints']['conf']
            features['f15'] = 1 if kps_conf[KEYPOINT_IDS['beaker']['bottom']] > KEYPOINT_VISIBILITY_THRESHOLD else 0
            features['f16'] = 1 if kps_conf[KEYPOINT_IDS['beaker']['mouth']] > KEYPOINT_VISIBILITY_THRESHOLD else 0
            features['f17'] = 1 if kps_conf[KEYPOINT_IDS['beaker']['spout']] > KEYPOINT_VISIBILITY_THRESHOLD else 0
        if features['f1'] and features['f2'] and beaker.get('keypoints') is not None and flask.get('keypoints') is not None:
            ...
            angle1, angle2 = get_line_angle(flask_pts_to_fit), get_line_angle(bk_pts_to_fit)
    ...
    # 步骤A, B, C, D
    all_beakers, all_flasks, all_hands = initial_detection(img, _MODELS_CACHE_BF['YOLO_MODELS']['box_model'], _MODELS_CACHE_BF['HANDS_DETECTOR'])
    selected_beaker, selected_flask = select_target_pair(all_beakers, all_flasks, all_hands)
    run_pose_detection(img, selected_beaker, 'beaker', _MODELS_CACHE_BF['YOLO_MODELS']['pose_models'])
    run_pose_detection(img, selected_flask, 'volumetric_flask', _MODELS_CACHE_BF['YOLO_MODELS']['pose_models'])
    feature_vector = extract_features(selected_beaker, selected_flask, all_hands)
    
    # 步骤E: 预测
    input_data = np.array(feature_vector).reshape(1, -1)
    prediction_probabilities = _MODELS_CACHE_BF['XGB_MODEL'].predict_proba(input_data)
